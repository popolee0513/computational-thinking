...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if capacity >= self.record[(x,y)]:
            self.record[(x,y)] = 0 
        else:
            self.record[(x,y)]-= capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.record[(m,n)] == 0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return sum([x==0 for x in list(self.record.values())])
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return len(self.record)- len(self.furniture_tiles)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    check = (room.get_num_cleaned_tiles() >=room.get_num_tiles()*min_coverage)
    ...
    for num in range(num_trials):
        ...
        room =  EmptyRoom(width, height, dirt_amount)
        check = (room.get_num_cleaned_tiles() >=room.get_num_tiles()*min_coverage)
        robot_list = [ robot_type(room, speed, capacity) for i in range(num_robots) ]
         
        
        while not check:
            [ x.update_position_and_clean() for x in robot_list]
            count +=1  
